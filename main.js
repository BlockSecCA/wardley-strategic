...
// src/export.ts
function exportToOWM(graph, contextManager, settings) {
...
function exportToSVG(graph, contextManager, settings) {
  const context = contextManager.getCurrentMapContext() || contextManager.getDefaultMapContext();
  if (!context)
    return null;
  const { components, edges } = buildPositionedMap(graph, contextManager, settings, context.id);
...
function buildPositionedMap(graph, contextManager, settings, mapId) {
  var _a;
  ...
  for (const id of componentIds) {
    ...
    components.push({
      id,
      name: ((_a = id.split("/").pop()) == null ? void 0 : _a.replace(".md", "")) || id,
      strategic: node.strategic,
      x: 0,
      y: 0
    });
...
function buildSVGString(components, edges, settings, title) {
  const lines = [];
  ...
  for (const edge of edges) {
    ...
    const dash = edge.type === "constrains" ? ' stroke-dasharray="5,5"' : edge.type === "evolves_to" || edge.type === "evolved_from" ? ' stroke-dasharray="3,3"' : "";
...
function clipLine(sx, sy, tx, ty, r) {
  const dx = tx - sx;
  const dy = ty - sy;
  const dist = Math.sqrt(dx * dx + dy * dy);
  if (dist <= r * 2)
